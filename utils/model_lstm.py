...
class LesionLatentLSTM(nn.Module):
    ...
    and uses a small LSTM (or GRU) over latent vectors instead of ConvLSTM over
    ...
    def forward(self, grids, times, patient_idx=None, teacher_forcing=True, n_future=0):
        ...
        # optionally append n_future placeholders (zeros) to times; times should already include n_future timestamps when requested
        total_steps = (T - 1) + n_future
        ...
        z_inputs = []
        for step in range(total_steps):
            # condition on the time we're predicting toward
            target_time = times[:, step + 1]
            t_enc = self.time_encoder(target_time)
            # take z at current step (observed) as part of input
            z_curr = z_obs[:, step]
