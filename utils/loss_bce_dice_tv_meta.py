...
class Loss_BCE_Dice_TV:
    def __init__(
        ...
        self.loss_bce_fn = torch.nn.BCEWithLogitsLoss(pos_weight=torch.tensor([self.pos_weight], device=device))
        self.loss_bce = 0.0
        self.loss_dice = 0.0
    ...
    def __call__(self, pred, target, coords=None):
        if self.use_dynamic_pos_weight:
            ...
            pw = torch.tensor(self.current_pos_weight, dtype=pred.dtype, device=pred.device)
            self.loss_bce_fn = torch.nn.BCEWithLogitsLoss(pos_weight=pw)
        else:
            pw = torch.tensor(self.pos_weight, dtype=pred.dtype, device=pred.device)
            self.loss_bce_fn = torch.nn.BCEWithLogitsLoss(pos_weight=pw)
            self.current_pos_weight = self.pos_weight

        self.loss_bce = self.loss_bce_fn(pred, target)
        self.loss_dice = self.dice_loss(pred, target, pos_weight=self.current_pos_weight)
        ...
        if coords is not None and (
            ...
            self.loss_tv = self.loss_tv_space + self.loss_tv_time

            total_loss += self.loss_tv
