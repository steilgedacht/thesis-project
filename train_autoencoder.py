...
def plot_autoencoder_reconstructions(model, epoch, plotting_LesionDataset, config):
    """Plot every plotting sample as a label/reconstruction montage.

    Each row contains one observed visit from one trajectory. Reconstructions
    are produced by the encoder and decoder only; the recurrent model is never
    called.
    """
    if not hasattr(model, "forward_autoencoder"):
        raise AttributeError("The configured model must provide forward_autoencoder().")

    rows = []
    for trj in plotting_LesionDataset:
        grids = torch.from_numpy(trj.grids).unsqueeze(0).float().to(config.device)
        with torch.no_grad():
            reconstructions = torch.sigmoid(model.forward_autoencoder(grids))

        labels = grids[0, 0, :].cpu().numpy()
        reconstructions = reconstructions[0, 0, :].cpu().numpy()
        for visit_idx, (label, reconstruction) in enumerate(zip(labels, reconstructions)):
            rows.append((
                f"{trj.patient_id} / lesion {trj.label_id} / "
                f"t={trj.times[visit_idx]:.0f}",
                label.max(axis=0),
                reconstruction.max(axis=0),
            ))
    ...
    figure, axes = plt.subplots(
        len(rows), 2,
        figsize=(8, max(3, 2.8 * len(rows))),
        squeeze=False,
    )
    figure.suptitle(f"Autoencoder reconstruction, epoch {epoch}")
    for row_idx, (row_label, label, reconstruction) in enumerate(rows):
        axes[row_idx, 0].imshow(label, cmap="gray", vmin=0, vmax=1)
        axes[row_idx, 1].imshow(reconstruction, cmap="magma", vmin=0, vmax=1)
        axes[row_idx, 0].set_ylabel(row_label, rotation=0, ha="right", va="center")
        axes[row_idx, 0].set_title("Label")
        axes[row_idx, 1].set_title("Encoder-decoder output")
        axes[row_idx, 0].axis("off")
        axes[row_idx, 1].axis("off")

    figure.tight_layout()
    output_path = f"/tmp/{epoch:04d}_epoch_autoencoder_reconstructions.png"
...
def train_lstm(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        plot_autoencoder_reconstructions(model, epoch, plotting_LesionDataset, config)
